export class ChatHistoryManager {
    ...
    addAction(action) {
        if (!this.chatRef.current.actions) {
            this.chatRef.current.actions = [];
        }
        
        this.chatRef.current.actions.push({
            ...action,
        ...
        if (this.updateFunction) {
            this.updateFunction(this.chatRef.current);
        }
    }

    addToolUsage(toolType, details) {
        if (!this.chatRef.current.toolUsage) {
            this.chatRef.current.toolUsage = [];
        }
        
        this.chatRef.current.toolUsage.push({
            type: toolType,
            details: details,
            timestamp: new Date().toISOString()
        });
