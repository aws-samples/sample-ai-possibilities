...
def initialize_agent():
    ...
    # Get configuration from environment - Now using Nova Omni
    model_id = os.getenv("BEDROCK_MODEL_ID", "global.amazon.nova-2-omni-v1:0")
    ...
    print(f"🤖 Using model: {model_id} (Nova Omni)")
    print(f"🌎 AWS Region: {region}")

    # Set up Bedrock model with Nova Omni
    bedrock_model = BedrockModel(
        model_id=model_id,
        region=region,
        temperature=temperature,  # Lower temperature for more consistent responses
        streaming=True,
        max_tokens=4096  # Nova Omni max tokens
    )
...
RESPONSE FORMATTING:
1. NEVER use markdown code blocks (```json, ```, etc.) in your responses - the UI cannot render them properly
2. When listing videos, describe them in natural language with clear formatting
3. Include the video_id for reference but format it naturally, like "Video ID: abc123"
4. Use bullet points, headings, and bold text for organization instead of code blocks
5. If you need to show structured data, use plain text formatting with clear labels
