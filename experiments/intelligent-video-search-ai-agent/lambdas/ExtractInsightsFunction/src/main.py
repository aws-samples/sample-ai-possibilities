...
class CohereEmbeddingClient:
    ...
    def generate_embeddings(self, texts: List[str], input_type: str = "search_document") -> List[float]:
        """
        Generate embeddings for marketing content using Cohere
    ...
    def generate_comprehensive_insights(self, video_id: str, index_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            summary_result = self.client.summarize(
                video_id=video_id,
                type="summary",
                prompt="Provide a comprehensive marketing analysis including brand visibility, emotional impact, key messages, and target audience appeal"
            )
            ...
            highlights_result = self.client.summarize(
                video_id=video_id,
                type="highlight",
                prompt="Identify key marketing moments, product reveals, emotional peaks, and call-to-action segments"
            )
            insights['highlights'] = [
                {
            ...
            metrics_result = self.client.analyze(
                video_id=video_id,
                prompt="Extract key marketing metrics: brand mentions count, call-to-action moments, product visibility duration, and audience engagement indicators"
            )
            insights['marketing_metrics'] = metrics_result.data
...
def prepare_marketing_content_for_embedding(insights: Dict[str, Any], detections: Dict[str, Any]) -> str:
    """
    Prepare the most relevant marketing content for Cohere embedding.
    This content will be used for semantic searches by marketing teams.
    """
    content_parts = []
    
    # 1. Summary - This is crucial for marketing searches
    ...
    # 8. Emotional content (for emotional marketing searches)
...
def lambda_extract_insights(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Prepare marketing content for Cohere embedding
        marketing_content = prepare_marketing_content_for_embedding(insights, search_results)
        
        # Generate Cohere embedding for marketing content
        logger.info("Generating Cohere embedding for marketing content")
        cohere_embeddings = cohere_client.generate_embeddings(
            texts=[marketing_content],
        ...
        video_data = {
            ...
            'pegasus_content_for_embedding': marketing_content,  # The text that was embedded
            ...
            # Marketing metadata (if provided)
            'marketing_metadata': event.get('marketing_metadata', {})
        }
        ...
        response = {
            ...
            'body': {
                **event,
                ...
                'marketing_content_length': len(marketing_content)
            }
        }
