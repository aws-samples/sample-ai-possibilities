...
PRIMARY_REGION = os.getenv("PRIMARY_REGION", "us-east-1")  # For Marengo and main resources
PEGASUS_REGION = os.getenv("PEGASUS_REGION", "us-west-2")  # For Pegasus model
AWS_REGION = PRIMARY_REGION  # Backward compatibility
OPENSEARCH_ENDPOINT = os.getenv("OPENSEARCH_ENDPOINT", "")
INDEX_NAME = os.getenv("INDEX_NAME", "video-insights-rag")

# Bedrock TwelveLabs Configuration (cross-region)
...
VIDEO_BUCKET_WEST = os.getenv('VIDEO_BUCKET_WEST')  # S3 bucket in us-west-2
...
def get_bedrock_config() -> Dict[str, str]:
    """Get Bedrock TwelveLabs configuration from environment variables"""
    config = {
        'marengo_model_id': MARENGO_MODEL_ID,
    ...
    if not config['video_bucket_west']:
        raise ValueError("VIDEO_BUCKET_WEST environment variable not set. Required for cross-region deployment.")
    
    logger.info(f"Using Bedrock config: {config}")
    return config
...
def get_bedrock_clients():
    """Get or create Bedrock client instances for cross-region TwelveLabs models"""
    ...
    if _bedrock_clients is None:
        try:
            ...
            _bedrock_clients = {
                'bedrock_runtime_east': boto3.client('bedrock-runtime', region_name=config['marengo_region']),
                'bedrock_runtime_west': boto3.client('bedrock-runtime', region_name=config['pegasus_region']),
                's3_client': boto3.client('s3'),
                'config': config
            }
            
            logger.info("Bedrock clients initialized successfully for cross-region TwelveLabs models")
...
def generate_video_embedding_from_s3(video_s3_uri: str, video_bucket: str) -> List[float]:
    """Generate video embedding using Bedrock Marengo model"""
    try:
        clients = get_bedrock_clients()
        bedrock_client = clients['bedrock_runtime_east']
        ...
        logger.info(f"Starting Marengo embedding generation for: {video_s3_uri}")
        ...
        while elapsed_time < max_wait_time:
            ...
            if status == 'Completed':
                logger.info("Marengo embedding generation completed")
                ...
                if 'Contents' in response:
                    for obj in response['Contents']:
                        key = obj['Key']
                        if key.endswith('.json') and 'output' in key:
                            # Download embeddings
                ...
                raise Exception("No valid embeddings found in Marengo output")
        ...
        raise Exception("Marengo embedding generation timed out")
...
def search_by_video_upload(
    ...
    try:
        ...
        # Generate video embedding using Bedrock Marengo
        video_s3_uri = f"s3://{temp_bucket}/{temp_s3_key}"
        logger.info("Generating video embedding with Bedrock Marengo...")
...
if __name__ == "__main__":
    ...
    if not OPENSEARCH_ENDPOINT:
        ...
        print(f"✓ Using Bedrock Models:")
        print(f"  - Marengo (Embeddings): {MARENGO_MODEL_ID} in {PRIMARY_REGION}")
        print(f"  - Pegasus (Understanding): {PEGASUS_MODEL_ID} in {PEGASUS_REGION}")
        print(f"  - Cohere (Text Embeddings): {COHERE_MODEL_ID}")
