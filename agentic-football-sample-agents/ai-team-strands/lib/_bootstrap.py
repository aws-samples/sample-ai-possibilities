...
Usage (at the very top of each agent's main.py):
    ...
    import os, sys; sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "lib")); from _bootstrap import setup_lib_path; setup_lib_path(__file__)
...
def setup_lib_path(caller_file: str) -> None:
    ...
    Works for both layouts:
      ...
      - Local dev: ai-team/lib/ is two levels up from agent/src/
    """
    here = os.path.dirname(os.path.abspath(caller_file))
    lib_deployed = os.path.join(here, "..", "lib")       # agent/lib  (AgentCore)
    lib_local = os.path.join(here, "..", "..", "lib")     # ai-team/lib (local dev)
