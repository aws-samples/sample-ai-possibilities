...
# ------ Attach memory permissions to execution role ------
echo "Attaching AgentCore Memory permissions to execution role..."
set +e
...
if [ -n "$EXEC_ROLE_NAME" ]; then
  aws iam put-role-policy \
    ...
    || echo "  ⚠️  Failed to attach memory permissions (may already exist)"
else
  echo "  ⚠️  Could not find execution role — attach AgentCoreMemoryAccess policy manually"
fi
echo ""
